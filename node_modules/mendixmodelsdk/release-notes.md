...
| story# | impact | description |
|---|---|---|
| MS-287 | Low | Disallow deleting detached elements. Either delete the element directly, or re-attach it before deleting it. |
| 1196423 | None | Added `filterUnitsByCustomWidgetId` to enable users with backend credentials find unit containing a specific custom widget. |
...
| MS-216 | None | Removed `getEditorData` internal API. |
...
| story# | impact | description |
|---|---|---|
| 1085587 | Breaking | Moving elements now has to be done though the `detach()` function. A full description of all states and their meaning is documented in [ModelSDK-States.md](../documentation/api-sdk/ModelSDK-States.md). This does not affect reading models and elements. |
| 1085587 | Breaking | Creating, deleting and changing elements is restricted by their _state_. A full description of all states and their meaning is documented in [ModelSDK-States.md](../documentation/api-sdk/ModelSDK-States.md). This does not affect reading models and elements. |
