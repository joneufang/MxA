...
var MendixSdkClient = (function () {
    ...
    function MendixSdkClient(username, apikey, password, openid, projectsApiEndpoint, modelApiEndpoint, options) {
        ...
        if (!(lodash_1.isEmpty(modelApiEndpoint))) {
...
var PlatformSdkClient = (function () {
     ...
     */
    PlatformSdkClient.prototype.createNewApp = function (projectName, projectSummary, templateUUID) {
        var _this = this;
        ...
        var apiClient = rest_1.wrap(this._createHttpErrorCodeInterceptor("Failed to create new app"))
    ...
    PlatformSdkClient.prototype.createOnlineWorkingCopy = function (project, revision) {
        ...
        var apiClient = rest_1.wrap(this._createHttpErrorCodeInterceptor("Failed to create online working copy"))
            .wrap(this._parseResult())
            .wrap(pathPrefix, { prefix: this._projectsApiEndpoint });
    ...
    PlatformSdkClient.prototype.commitToTeamServer = function (workingCopy, branchName, baseRevision) {
        ...
        var teamServerInfo = "team server project " + workingCopy.project().id() + " branch " + branchName + " base revision " + baseRevision;
        ...
        var apiClient = rest_1.wrap(this._createHttpErrorCodeInterceptor("Failed to commit to team server"))
            .wrap(this._parseResult())
            .wrap(pathPrefix, { prefix: this._projectsApiEndpoint });
        return apiClient(request)
            .then(function (response) {
            ...
            .then(function (jobResult) {
            return when_1.promise(function (resolve, reject) {
                var num = parseInt(jobResult.result, 10);
                if (num === null) {
    ...
    PlatformSdkClient.prototype._awaitJobResult = function (jobId) {
        ...
        return when_1.promise(function (resolve, reject) {
            setTimeout(function () {
                ...
                var client = rest_1.wrap(_this._createHttpErrorCodeInterceptor("Error when retrieving job status"))
    ...
    PlatformSdkClient.prototype._compilePayload = function (templateName, data) {
        var xmlPayloadTemplate = fs.readFileSync(templateName, "utf8");
        var compileXmlPayload = lodash_1.template(xmlPayloadTemplate);
            ...
            return when_1.promise(function (resolve, reject) {
                if (lodash_1.isEmpty(response.status) || lodash_1.isEmpty(response.entity)) {
                    ...
                    _this._parseAndQuery(response.entity, "$..faultstring[0]")
                        .done(function (cause) { return reject(errorMessage + ": " + cause.replace(/\\[\r\n]+/g, os.EOL)); }, function (error) { return reject(error); });
                }
                else {
                    var retryMessage = "Please retry after a few minutes. If the problem persists, please consult https://mxforum.mendix.com";
...
PlatformSdkClient.PROJECTS_API_PATH = "/ws/ProjectsAPI/10/soap1";
...
function loadAsPromise(loadable) {
    return when_1.promise(function (resolve, reject) { return loadable.load(resolve); });
}
